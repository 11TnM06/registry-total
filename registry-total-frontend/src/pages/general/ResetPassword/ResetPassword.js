...
function ResetPassword(props) {
  ...
  const resetInfo = React.useMemo(
    () => ({
      token: resetToken,
      password: resetPassword,
      retypePassword: resetRetypePassword,
    }),
    [resetToken, resetPassword, resetRetypePassword]
  );
  ...
  const validReset = () => {
    return (
      UseValidation.token(resetToken).state &&
      UseValidation.password(resetPassword).state &&
      validRetypePassword(resetRetypePassword).state
    );
  };
  ...
  return (
    <Form width="25rem">
      ...
      <Form.Notify enabled={true}>
        Chúng tôi đã gửi mã xác minh gồm 6 chữ số tới email{" "}
        <span style={{ color: "green" }}>{email}</span>. Hãy nhập mã đó để xác
        minh email của bạn
      </Form.Notify>
      <Form.Input
        label="Mã xác minh"
        type="text"
        reference={[resetToken, setResetToken, UseValidation.token]}
      />
      <Form.Input
        label="Mật khẩu mới"
        type="password"
        reference={[resetPassword, setResetPassword, UseValidation.password]}
      />
      <Form.Input
