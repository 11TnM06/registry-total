...
function ChangePassword(props) {
  ...
  const validChangẻPassword = () => {
    return (
      UseValidation.loginPassword(currentPassword).state &&
      UseValidation.password(newPassword).state &&
      validRetypePassword(confirmPassword).state
    );
  };

  const onChangePassword = () => {
    UseFetch(`/api/user/change_password`, "POST", {
  ...
  return (
    <Form>
      ...
      <Form.Input
        label="Mật khẩu mới"
        type="password"
        reference={[newPassword, setNewPassword, UseValidation.password]}
      />
      <Form.Input
        label="Nhập lại mật khẩu mới"
        type="password"
        reference={[confirmPassword, setConfirmPassword, validRetypePassword]}
      />
      <Form.Error enabled={error !== ""} content={error} />
      <Form.Submit
        content="Đổi mật khẩu"
        validation={validChangẻPassword}
