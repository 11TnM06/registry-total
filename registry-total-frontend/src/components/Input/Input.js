...
function Input(props) {
  ...
  return (
    <div className="input-wrapper">
      ...
      <label className="label" disabled={props.disabled}>
        {props.label}
      </label>
      ...
      <input
        ...
        onInput={(event) => {
          if (props.reference && props.reference[2])
            displayValidation(event, props.reference[2]);
          if (props.reference && props.reference[1])
      ...
      <p style={{ display: error === "" ? "none" : "block" }} className="error">
        {error}
      </p>
    </div>
  );
