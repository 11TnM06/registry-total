...
const Table = forwardRef((props, ref) => {
  ...
  const [displayColumn, setDisplayColumn] = React.useState([
    ...Object.keys(props.data.length ? props.data[0] : []).map(() => true),
  ]);

  const [tableData, setTableData] = React.useState({
    data: [
      ...props.data.map((s, i) => Object.assign({}, { __data_order: i }, s)),
    ],
    selected: [],
  });
  ...
  useImperativeHandle(ref, () => ({
    ...
    updateTable(newRow, method) {
      var index, newData;
      if (method === "add") {
        var newRowObject = Object.assign(
          {},
          { __data_order: tableData.data.length },
          newRow
        );
        setTableData({
          data: [...tableData.data, newRowObject],
          selected: tableData.selected,
        });
        setTablePage({
          page: Math.ceil(props.data.length / tablePage.pageSize),
    ...
    updateAllTable(newData) {
      setTableData({
        data: newData.map((s, i) => Object.assign({}, { __data_order: i }, s)),
        selected: [],
      });
      setTablePage({ page: 1, pageSize: tablePage.pageSize });
      setDisplayColumn([
        ...Object.keys(newData.length ? newData[0] : []).map(() => true),
      ]);
    },
    ...
    const sortTable = (col, order) => {
      ...
      if (order === 0) {
        setTableData({
          data: [
            ...props.data.map((s, i) =>
              Object.assign({}, { __data_order: i }, s)
            ),
          ],
          selected: tableData.selected,
        });
  ...
  return (
    ...
    >
      ...
      {props.multiTitle && (
        <div className="table-multi-title">{props.multiTitle}</div>
      )}
      {props.data.length === 0 ? (
