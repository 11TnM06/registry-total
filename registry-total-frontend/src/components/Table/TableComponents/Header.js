...
function TableHeaderCell(props) {
  ...
  return props.checkbox ? (
    ...
    <th
      ref={thRef}
      onMouseOver={() => displayFakeIcon(true)}
      onMouseOut={() => displayFakeIcon(false)}
    >
      <span>{UseTranslator.translate(props.label)}</span>
      <div className="th-icon">
        {props.order && props.order.sortColumn === thRef.current ? (
          props.order.sortOrder === 1 ? (
            <TableComponent.Icon.SortAsc />
          ) : props.order.sortOrder === 2 ? (
            <TableComponent.Icon.SortDesc />
          ) : (
            fakeIconComponent()
          )
        ) : (
      ...
      <tr>
        {props.checkbox && (
          <TableHeaderCell
            checkbox
            checked={
              props.tableData.selected.length === props.tableData.data.length
                ? 1
                : props.tableData.selected.length === 0
                ? 0
                : 2
            }
            onClick={() => onSelectAll()}
        ...
        {props.data.length > 0 &&
          Object.keys(props.data[0]).map((v, i) => {
            if (v === "id" && props.displayColumn[i])
              return <th key={v}>Stt.</th>;
            if (v !== "__data_order" && props.displayColumn[i])
              return <TableHeaderCell key={i} label={v} order={props.sort} />;
            else return null;
          })}
