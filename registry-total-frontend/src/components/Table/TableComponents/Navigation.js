      ...
      <Dropdown.MenuWrapperResponsive limitToBorder>
        <Section title="Chọn cột hiển thị">
          {props.columns.map((v, i) => {
            if (v === "id") {
              return (
                <Form.SplitFlex key={i} format="dashed-bottom">
            ...
            return (
              <Form.SplitFlex key={i} format="dashed-bottom">
                <Checkbox
                  checked={props.displayColumn.displayColumn[i] ? 1 : 0}
                  onClick={() => onDisplayColumn(i)}
                ></Checkbox>
                <span className="setting-column">
                  {UseTranslator.translate(v)}
                </span>
              </Form.SplitFlex>
            );
          })}
...
Navigation.Filter = function NavigationFilter(props) {
  ...
  const onFilter = (text) => {
    ...
    props.setTableData({
      data: [
        ...filtered.map((s, i) => Object.assign({}, { __data_order: i }, s)),
      ],
      selected: props.tableData.selected,
    });
    ...
    <Dropdown>
      ...
      <Dropdown.MenuWrapperResponsive limitToBorder>
        <Section title="Lọc">
          <Input
            type="text"
            label="Giá trị cần lọc"
            reference={[filterText, onFilter]}
          />
        </Section>
      </Dropdown.MenuWrapperResponsive>
    </Dropdown>
