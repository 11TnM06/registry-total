...
UsePreprocessChart.Admin = function (data, name, type) {
  if (!data) return null;
  if (type == "Năm") {
...
UsePreprocessChart.Line = function (data, name, type) {
  ...
  if (type === 0) {
    ...
    for (
      let year = parseInt(key[0]);
      year <= parseInt(key[key.length - 1]);
      year++
    ) {
      const key = `${year}`;
      newDataKey.push(key);
...
UsePreprocessChart.Mix = function (data, name1, name2, type) {
  ...
  if (type === 0) {
    ...
    return {
      options: {
        xaxis: { categories: newDataKey },
        yaxis: [{}, { opposite: true }],
      },
      series: [
        { name: name1, type: "column", data: newDataValue1 },
        { name: name2, type: "line", data: newDataValue2 },
      ],
    };
  } else if (type === 1) {
    for (let year = firstVal; year <= lastVal; year++) {
