...
function username(input) {
  const usernamePattern = /^(?=[a-zA-Z0-9._]{5,20}$)(?!.*[_.]{2})[^_.].*[^_.]$/;
  if (input.length === 0) {
    return { state: false, content: "Tên đăng nhập không được để trống" };
    ...
    return {
      state: false,
      content:
        "Tên đăng nhập phải có độ dài từ 5 đến 20 kí tự và không chứa kí tự đặc biệt",
    };
  }
  return { state: true };
}
...
function password(input) {
  ...
  if (input.length === 0) {
    ...
    return {
      state: false,
      content:
        "Mật khẩu phải có ít nhất 1 chữ hoa, 1 chữ thường, 1 số và có độ dài từ 7 đến 20 kí tự",
    };
  }
  return { state: true };
}
...
function name(input) {
  const productNamePattern =
    /^[0-9a-zA-Z /ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂưăạảấầẩẫậắằẳẵặẹẻẽềềểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹý]+$/;
  if (input.length === 0) {
    return { state: false, content: "Tên không được để trống" };
  } else if (!productNamePattern.test(input)) {
    return {
      state: false,
      content: "Tên chỉ được chứa số, chữ cái và khoảng trắng",
    };
  }
  return { state: true };
}
...
function companyName(input) {
  ...
  if (input.length === 0) {
    ...
    return {
      state: false,
      content: "Tên công ty chỉ được chứa số, chữ cái và khoảng trắng",
    };
  }
  return { state: true };
}
...
function customerName(input) {
  const customerNamePattern =
    /^[a-zA-Z ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀẾỂưạảấầẩẫậắằẳẵặẹẻẽềếểỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹý]+$/;
  if (input.length === 0) {
    return { state: false, content: "Tên khách hàng không được để trống" };
  } else if (!customerNamePattern.test(input)) {
    return {
      state: false,
      content: "Tên khách hàng chỉ được chứa chữ cái và khoảng trắng",
    };
  }
  return { state: true };
}
...
function productName(input) {
  ...
  if (input.length === 0) {
    ...
    return {
      state: false,
      content: "Tên sản phẩm chỉ được chứa số, chữ cái và khoảng trắng",
    };
  }
  return { state: true };
}
...
function price(input) {
  ...
  if (input.length === 0) {
    ...
    return {
      state: false,
      content: "Giá sản phẩm chỉ được chứa số và dấu chấm",
    };
  }
  return { state: true };
}
...
function required(input) {
  if (input.length === 0) {
    return { state: false, content: "Không được để trống" };
  }
  return { state: true };
}
